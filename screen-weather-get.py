...
def format_weather_description(weather_description):
    if len(weather_description) < 18:
        return {1: weather_description, 2: ''}

    splits = textwrap.fill(weather_description, 18, break_long_words=False,
...
def get_weather_current(location_lat, location_long, units):
    ...
    if (
        ...
        logging.error("No weather provider has been configured (Climacell, OpenWeatherMap, Weather.gov, MetOffice, AccuWeath>        sys.exit(1)
    ...
    weather_current = weather_provider.get_weather_current()
    logging.info("weather - {}".format(weather_current))
    return weather_current

def get_weather_forecast(location_lat, location_long, units):
    ...
    weather_forecast = weather_provider.get_weather_forecast()
    logging.info("weather - {}".format(weather_forecast))
...
def main():
    ...
    weather_current = get_weather_current(location_lat, location_long, units)

    if not weather_current:
        logging.error("Unable to fetch weather payload. SVG will not be updated.")
        return

    weather_desc_current = format_weather_description(weather_current["current_description"])

    weather_forecast = get_weather_forecast(location_lat, location_long, units)

    if not weather_forecast:
        logging.error("Unable to fetch weather payload. SVG will not be updated.")
        return

    weather_desc_forecast = format_weather_description(weather_forecast["forecast_description"])
    ...
    daytime = weather_forecast['is_it_daytime']
    if daytime == True:
        today_tonight = "Today:"
    else:
        today_tonight = "Tonight:"

    output_dict = {
        'TEMP_NOW': "{}{}".format(str(round(weather_current['current_temperature'])), degrees),
        'ICON_ONE': weather_current["current_icon"],
        'WEATHER_DESC_1': weather_desc_current[1],
        'WEATHER_DESC_2': weather_desc_current[2],
        'LOW_ONE': "{}{}".format(str(round(weather_forecast['forecast_temperatureMin'])), degrees),
        'HIGH_ONE': "{}{}".format(str(round(weather_forecast['forecast_temperatureMax'])), degrees),
        'ICON_TWO': weather_forecast["forecast_icon"],
        'WEATHER_DESC_3': weather_desc_forecast[1],
        'WEATHER_DESC_4': weather_desc_forecast[2],
        'TIME_NOW_FONT_SIZE': time_now_font_size,
        'TIME_NOW': time_now,
        'TODAY_TONIGHT': today_tonight,
